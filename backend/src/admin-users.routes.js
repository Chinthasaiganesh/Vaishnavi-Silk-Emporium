...
router.get("/users", authRequired, adminOnly, (req, res) => {
  const users = db.prepare("SELECT * FROM Users WHERE Role = 'ADMIN' ORDER BY datetime(CreatedDate) DESC").all();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = db.prepare("INSERT INTO Users (Username, PasswordHash, Role, FullName, DisplayName, Email, MobileNumber, CreatedDate, IsEnabled) VALUES (?, ?, 'ADMIN', ?, ?, ?, ?, ?, 1)").run(req.body.username, passwordHash, req.body.fullName, req.body.displayName, req.body.email, req.body.mobileNumber || "", nowIso());
...
router.put(
  ...
  async (req, res) => {
    ...
    const target = db.prepare("SELECT * FROM Users WHERE UserId = ? AND Role = 'ADMIN'").get(targetId);
    ...
    try {
      db.prepare("UPDATE Users SET FullName = ?, DisplayName = ?, Email = ?, MobileNumber = ?, IsEnabled = ?, PasswordHash = ? WHERE UserId = ?").run(req.body.fullName, req.body.displayName, req.body.email, req.body.mobileNumber || "", isEnabled ? 1 : 0, passwordHash, targetId);
      if (!isEnabled || req.body.newPassword) db.prepare("DELETE FROM RefreshSessions WHERE UserId = ?").run(targetId);
      return res.json({ user: mapAdmin(db.prepare("SELECT * FROM Users WHERE UserId = ?").get(targetId)) });
    } catch (error) {
...
router.delete("/users/:id", authRequired, adminOnly, param("id").isInt({ min: 1 }), validateRequest, (req, res) => {
  const targetId = Number(req.params.id);
  if (targetId === req.user.userId) return res.status(400).json({ message: "You cannot delete your own admin account." });
  const result = db.prepare("DELETE FROM Users WHERE UserId = ? AND Role = 'ADMIN'").run(targetId);
