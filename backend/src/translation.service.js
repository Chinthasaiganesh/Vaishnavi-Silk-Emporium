...
export async function translateText(text, { sourceLanguage = "en", targetLanguage = "te" } = {}) {
  ...
  const cached = db.prepare("SELECT TranslatedText, Provider FROM TranslationCache WHERE CacheKey = ?").get(key);
  ...
  db.prepare("INSERT INTO TranslationCache (CacheKey, SourceLanguage, TargetLanguage, SourceText, TranslatedText, Provider, CreatedDate) VALUES (?, ?, ?, ?, ?, ?, ?)").run(key, sourceLanguage, targetLanguage, text, translatedText, provider, nowIso());
